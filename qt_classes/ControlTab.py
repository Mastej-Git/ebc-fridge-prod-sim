...
class ControlTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.read_file_button = AnimatedButton("Load")
        self.chose_file_button = AnimatedButton("Chose a file")

        vbox_layout.addWidget(self.chose_file_button)
        vbox_layout.addWidget(self.read_file_button)
        ...
        self.label_quantity = self.create_label("No quantity selected")
    ...
    def show_quantity_dialog(self):
        value, ok = QInputDialog.getInt(
            ...
            value=self.selected_quantity,
            min=0,
            max=9999,
            step=1
        )
        if ok:
            self.selected_quantity = value
            self.label_quantity.setText(f"Quantity selected: {value}")
